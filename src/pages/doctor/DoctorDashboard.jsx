                ...
                <div className="grid grid-cols-1 lg:grid-cols-10 gap-4 mt-4">
                  ...
                  <div className="lg:col-span-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex flex-col h-full">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      }} className="text-[14px] font-bold text-teal-600 hover:underline">Edit</button>
                    </div>

                    <div className="grid grid-cols-2 gap-1.5 flex-grow" onClick={() => {
                      setVitalsInput({
                        bloodPressure: patientData?.vitals?.[0]?.bloodPressure || '',
                        sugarLevel: patientData?.vitals?.[0]?.sugarLevel || '',
                        pulseRate: patientData?.vitals?.[0]?.pulseRate || '',
                        ...
                        <div className="divide-y divide-gray-50">
                          {filteredQueue.map((patient, idx) => (
                            <div key={patient._id}
                              ...
                              <span className={`shrink-0 px-2 py-0.5 rounded text-[14px] font-bold uppercase tracking-wider hidden md:block ${patient.status === 'In-Consultation' ? 'bg-green-50 text-green-600 border border-green-100' :
                    ...
                    <div className="hidden lg:block rounded-xl p-4 text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 60%, #134e4a 100%)' }}>
                      ...
                      <div className="relative z-10">
                          ...
                          <div className="flex gap-1.5 mb-2.5">
                            {[5, 10, 15].map(m => (
                              <button
                                ...
                                className={`flex-1 py-1 rounded-lg text-[14px] font-black uppercase tracking-wider transition-all border ${selectedBreakMins === m
                        ...
                        <button
                          ...
                          className={`w-full py-2 rounded-lg font-black text-[14px] uppercase tracking-wider transition-all active:scale-[0.97] ${isOnBreak
                    ...
                    <div className="flex gap-6 overflow-x-auto pb-2 scrollbar-hide">
                      {queue.filter(app => app.visitType === 'Appointment').map((app, idx, arr) => (
                        <div key={app._id} className="flex-shrink-0 flex items-start gap-2.5 relative min-w-[160px]">
                          ...
                          <div className={`w-4 h-4 rounded-full border-[3px] border-white shadow-md z-10 mt-0.5 shrink-0 ${app.status === 'In-Consultation' ? 'bg-green-500' : app.status === 'Waiting' ? 'bg-orange-400' : app.status === 'Completed' ? 'bg-blue-500' : 'bg-gray-300'
                            }`} />
                          <div>
                            ...
                            <span className={`px-1.5 py-0.5 rounded text-[14px] font-bold uppercase tracking-wider ${app.status === 'In-Consultation' ? 'bg-green-50 text-green-600' : app.status === 'Waiting' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'
                              }`}>{app.status}</span>
                          </div>
                        </div>
                      ))}
